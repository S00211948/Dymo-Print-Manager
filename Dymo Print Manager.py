...
class Employee():
    ...
    # constructor
    def __init__(self, emp, g1, g2, g3, tour):
        self.Employee = emp
        self.Guest_1 = g1
        self.Guest_2 = g2
        self.Guest_3 = g3
...
class DymoPrintManager(tk.Tk):
    ...
    def delete_contact(self, index, edit_window):
...
def parse_csv(file_path):
    ...
    for row in csv.DictReader(open(file_path, encoding='utf-8-sig')):
        employees.append(Employee(f"{row['First_Name']} {row['Surename']}",row['Guest_1'],row['Guest_2'],row['Guest_3'],row['Tour_Number']))
